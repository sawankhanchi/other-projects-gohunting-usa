...
export const initiateOrder = (orderParams, transactionId) => (dispatch, getState, sdk) => {
  ...
  const bookingData = {
    ...
    units:orderParams.units?orderParams.units:1,
    seats:orderParams.seats?orderParams.seats:1,
    unitType:orderParams.unitType?orderParams.unitType:config.bookingUnitRegular,
    customPackages: orderParams.customPackages,
  };
console.log("order params", bookingData)
  const bodyParams = isTransition
    ...
    : {
        processAlias: orderParams.unitType?config.bookingProcessSeasonalAlias:config.bookingProcessAlias,
        transition,
        params: orderParams,
      };
  const queryParams = {
...
export const speculateTransaction = (orderParams, transactionId) => (dispatch, getState, sdk) => {
  ...
  if (isTransition && isPrivilegedTransition) {
    ...
    // transition non-privileged
    return sdk.transactions
      .transitionSpeculative({ isSpeculative: true, bookingData, bodyParams, queryParams })
      .then(handleSuccess)
      .catch(handleError);
  } else if (isPrivilegedTransition) {
    // initiate privileged
