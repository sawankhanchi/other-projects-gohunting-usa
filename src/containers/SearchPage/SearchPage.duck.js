...
export const searchListings = searchParams => (dispatch, getState, sdk) => {
  ...
  const areaSearchParams = areaParam => {
    const inSubunits = value =>
      convertUnitToSubUnit(value, unitDivisor(config.currencyConfig.currency));
    const values = areaParam ? areaParam.split(',') : [];
    return areaParam && values.length === 2
      ? {
          area: [inSubunits(values[0]), inSubunits(values[1]) + 1].join(','),
        }
      : {};
  };
  ...
  const { perPage, price, area, dates, ...rest } = searchParams;
  const priceMaybe = priceSearchParams(price);
  const areaMaybe = areaSearchParams(area);
