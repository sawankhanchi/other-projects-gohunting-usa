...
const LineItemBasePriceMaybe = props => {
  ...
  const quantity = (isNightly || isDaily)? (unitPurchase && unitPurchase.units? unitPurchase.units.toString(): null):(unitPurchase && unitPurchase.seats? unitPurchase.seats.toString(): null) ;
  const unitPrice = unitPurchase ? formatMoney(intl, unitPurchase.unitPrice) : null;
  const total = unitPurchase ? formatMoney(intl, unitPurchase.lineTotal) : null;
  const perPerson = seats>1 && (isNightly || isDaily) && <FormattedMessage id={translationKeyperson} values={{ unitPrice, seats }} />
