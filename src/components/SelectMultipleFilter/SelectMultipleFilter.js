...
const GroupOfFieldCheckboxes = props => {
  const { id, className, name, options, children } = props;
  return (
...
class SelectMultipleFilter extends Component {
  ...
  render() {
    ...
    console.log("jay amenities", options)
    ...
    const subOptions = options.map(amnities=> <GroupOfFieldCheckboxes
      className={css.fieldGroupPlain}
      name={name}
      id={`${id}-checkbox-group`}
      options={amnities.amenitiesOption}
    />);
    console.log("jay group",subOptions )
    return showAsPopup ? (
      <FilterPopup
        className={classes}
