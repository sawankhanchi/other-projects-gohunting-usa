...
const nonSiteUsersMaybe = config.enableNonsiteusers ? [NONSITEUSERS] : [];
...
const tabLabel = (intl, tab) => {
  ...
  } else if (tab === NONSITEUSERS) {
    key = 'EditListingWizard.tabLabelNonSiteUsers';
  }
...
class EditListingWizard extends Component {
  ...
  render() {
    ...
    const isSeasonal = currentListing && currentListing.attributes.publicData && currentListing.attributes.publicData.type === 'seasonal' ? true : false;
    if(!isSeasonal && TABS.indexOf('nonSiteUsers')>-1)
    TABS.splice(TABS.indexOf('nonSiteUsers'),1);
