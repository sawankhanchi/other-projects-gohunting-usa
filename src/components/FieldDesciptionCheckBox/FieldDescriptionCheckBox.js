...
import css from './FieldDescriptionCheckBox.module.css';
...
const FieldDescriptionCheckBoxComponent = props => {
  const {
    rootClassName,
    className,
    svgClassName,
  ...
  const handleOnChange = (input, event) => {
    ...
    onSelected({label,price: new Money(price*100, currency)}, !input.checked);
    onChange(event);
    onBlur(event);
  ...
  return (
    <div className={css.checkboxCntainer}>
...
FieldDescriptionCheckBoxComponent.defaultProps = {
  className: null,
  rootClassName: null,
  svgClassName: null,
  textClassName: null,
...
FieldDescriptionCheckBoxComponent.propTypes = {
  className: string,
...
export default FieldDescriptionCheckBoxComponent;
