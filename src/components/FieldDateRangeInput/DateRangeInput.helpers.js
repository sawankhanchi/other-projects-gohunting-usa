...
export const isDayBlockedFn = (timeSlots, startDate, endDate, focusedInput, unitType, seats) => {
  ...
  if (selectingEndDate) {
    ...
    return day =>
      !timeSlots.find(timeSlot => timeSlot.attributes.seats>= seats?timeSlotEqualsDay(timeSlot, moment(day).subtract(1, 'days')):false);
    ...
    return day => !timeSlots.find(timeSlot => timeSlot.attributes.seats>= seats? timeSlotEqualsDay(timeSlot, day): false);
  }
};
