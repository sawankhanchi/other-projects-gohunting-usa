...
export const ManageListingCardComponent = props => {
  ...
  const isSeasonal = currentListing.attributes.publicData && currentListing.attributes.publicData.type && currentListing.attributes.publicData.type === 'seasonal'? true: false;
  // console.log("jay type", currentListing.attributes.publicData, isSeasonal)
  ...
  const unitTranslationKey = isNightly
    ...
    const maxNoOfSeats = currentListing.attributes.publicData.maximumNumberOfSeatsPerDay ? currentListing.attributes.publicData.maximumNumberOfSeatsPerDay : null;
    // console.log(maxNoOfSeats)

    const nonSiteUsersLength = maxNoOfSeats && currentListing.attributes.publicData.nonSiteUsers ? currentListing.attributes.publicData.nonSiteUsers.length : null;
    // console.log(nonSiteUsersLength)
  ...
  return (
    <div className={classes}>
      ...
      {
        listing.attributes.state === "published" && <div className={css.editListingLink} >
                <label className={css.editIcon} >
                  Current Seats available ({maxNoOfSeats - nonSiteUsersLength})
                </label>
        </div>
      }
          ...
          {!isDraft && isSeasonal? (
            <React.Fragment>
              ...
              <NamedLink
                className={css.manageLink}
                name="EditListingPage"
                params={{ id, slug, type: editListingLinkType, tab: 'nonSiteUsers' }}
              >
                <FormattedMessage id="ManageListingCard.manageNonSiteUsers" />
                {listing.attributes.publicData.nonSiteUsers?.length && listing.attributes.publicData.nonSiteUsers?.length>0 && '(' + listing.attributes.publicData.nonSiteUsers?.length + ')'}
...
ManageListingCardComponent.defaultProps = {
  ...
  nonSiteUsersEnabled: config.enableNonSiteUsers,
};
