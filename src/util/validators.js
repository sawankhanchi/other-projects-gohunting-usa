...
export const TimeStampDateRequired = inValidDateMessage => value => {
  var dateIsValid = (new Date(value)).getTime() > 0;
  return !dateIsValid ? inValidDateMessage : VALID;
};
...
export const requiredNumber = (message, max) => value => {
  message = message.replace('{}',max)
  if(isNonEmptyString(value))
   return isNaN(value) === false && parseInt(value)<=max? VALID : message;
  else
  return VALID;
};
export const minmaxDateRange = (minMessage,maxMessage, min, max) => value => {
